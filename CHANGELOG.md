...
## 1.0.1.1
Fix the empty library dropdown (libraries now come from GetVirtualFolders). Redesigned the config page into a native Jellyfin two-column layout with grouped cards and a sticky live preview that updates automatically as you adjust settings — no more manual preview button. Styled the file pickers as native buttons and made the allowed formats and size limits explicit. Backfilled version changelogs.
